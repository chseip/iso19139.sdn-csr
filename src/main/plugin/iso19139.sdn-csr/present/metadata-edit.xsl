...
<xsl:stylesheet version="2.0" xmlns:xsl ="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- main template - the way into processing iso19139.sdn-csr -->
  ...
  <xsl:template mode="iso19139.sdn-csr" match="*|@*">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:variable name="empty">
          <xsl:apply-templates mode="iso19139.sdn-csrIsEmpty" select="."/>
        </xsl:variable>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:identificationInfo|gmd:distributionInfo|
              ...
              *[name(..)='gmd:resourceConstraints']|gmd:spatialRepresentationInfo|gmd:pointOfContact|
              ...
              gmd:processStep|gmd:lineage|gmi:acquisitionInformation">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:*[gco:Date|gco:DateTime|gco:Integer|gco:Decimal|gco:Boolean|gco:Real|gco:Measure|gco:Length|gco:Distance|gco:Angle|gco:Scale|gco:RecordType|gmx:MimeFileType]|
    ...
    <xsl:call-template name="iso19139.sdn-csrString">
      <xsl:with-param name="schema" select="$schema"/>
      <xsl:with-param name="edit"   select="$edit"/>
    </xsl:call-template>
  </xsl:template>
  ...
  -->
  <xsl:template mode="iso19139.sdn-csr" match="gmd:distance" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
      ...
      <xsl:when test="$edit=true()">
        <xsl:variable name="text">
          ...
          <div>
            ...
            &#160;
            <xsl:value-of select="/root/gui/schemas/iso19139.sdn-csr/labels/element[@name = 'uom']/label"/>
            &#160;
  ...
  <!--
    ...
    -->
  <xsl:template mode="iso19139.sdn-csr" match="srv:operatesOn|gmd:featureCatalogueCitation|gmd:source[name(parent::node())='gmd:LI_ProcessStep' or name(parent::node())='gmd:LI_Lineage']" priority="99">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    <xsl:variable name="text">
...
<!--<xsl:apply-templates mode="iso19139.sdn-csr" select="*">
   ...
   Display a list of related resources to which the current service metadata operatesOn.
     ...
     template in metadata-iso19139.sdn-csr-utils.xsl).
      -->
    <xsl:template mode="iso19139.sdn-csr" match="srv:coupledResource/srv:SV_CoupledResource/srv:identifier" priority="200">
        <xsl:param name="schema"/>
        <xsl:param name="edit"/>
  ...
  -->
  <xsl:template mode="iso19139.sdn-csr" match="gts:TM_PeriodDuration|gml:duration" priority="100">
    <xsl:param name="schema" />
    <xsl:param name="edit" />
  ...
  <xsl:template name="iso19139.sdn-csrString">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gco:ScopedName|gco:LocalName">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <!-- GML time interval -->
  <xsl:template mode="iso19139.sdn-csr" match="gml:timeInterval">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    ...
    <xsl:choose>
      <xsl:when test="$edit">
        <xsl:apply-templates mode="simpleElement" select=".">
          <xsl:with-param name="schema"   select="$schema"/>
          <xsl:with-param name="edit"     select="$edit"/>
          <xsl:with-param name="title"    select="/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gml:timeInterval']/label"/>
        </xsl:apply-templates>
      </xsl:when>
      <xsl:otherwise>
        <xsl:variable name="text">
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gml:identifier|gml:axisDirection|gml:descriptionReference">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <!-- gmx:FileName could be used as substitution of any
    ...
    suggestion.
    -->
  <xsl:template mode="iso19139.sdn-csr" name="file-upload" match="*[gmx:FileName]">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:contactInstructions[gmx:FileName]" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
        ...
        <xsl:choose>
          <xsl:when test="$edit">
            ...
            <xsl:if test="$delButton">
              <xsl:apply-templates mode="iso19139.sdn-csrFileRemove" select="gmx:FileName">
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:*[*/@codeList]|srv:*[*/@codeList]">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    ...
    <xsl:choose>
      <xsl:when test="$multiple=true()">
        ...
        <xsl:if test="$firstElement">
          <xsl:call-template name="iso19139.sdn-csrCodelistMultiple">
            <xsl:with-param name="schema" select="$schema"/>
            <xsl:with-param name="edit"   select="$edit"/>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:otherwise>
        <xsl:call-template name="iso19139.sdn-csrCodelist">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="edit"   select="$edit"/>
        </xsl:call-template>
      </xsl:otherwise>
  ...
  <xsl:template name="iso19139.sdn-csrCodelist">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    <xsl:apply-templates mode="simpleElement" select=".">
      <xsl:with-param name="schema" select="$schema"/>
      <xsl:with-param name="edit"   select="$edit"/>
      <xsl:with-param name="text">
        <xsl:apply-templates mode="iso19139.sdn-csrGetAttributeText" select="*/@codeListValue">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="edit"   select="$edit"/>
        </xsl:apply-templates>
      </xsl:with-param>
  ...
  -->
  <xsl:template name="iso19139.sdn-csrCodelistMultiple">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  -->
  <xsl:template mode="iso19139.sdn-csr" match="gmd:LanguageCode" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    
    <xsl:call-template name="iso19139.sdn-csrGetIsoLanguage">
  ...
  and iso code.-->
  <xsl:template mode="iso19139.sdn-csr" match="gmd:PT_Locale/@id"
    priority="2">
    <xsl:param name="schema" />
    <xsl:param name="edit" />
  ...
  <!-- 
    ...
    is returned if found. If not, the iso19139.sdn-csr codelist is returned.
  -->
  <xsl:function name="geonet:getCodelist" as="node()">
    ...
    <xsl:variable name="codelistProfil">
      <xsl:choose>
        <xsl:when test="starts-with($schema,'iso19139.sdn-csr.')">
          <xsl:copy-of
    ...
    <xsl:variable name="codelistCore">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:copy-of
            select="$labels/schemas/*[name(.)='iso19139.sdn-csr']/codelists/codelist[@name = $qname and not(@displayIf)]" />
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
  ...
  <xsl:template mode="iso19139.sdn-csrGetAttributeText" match="@*">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:dateStamp|gmd:fileIdentifier" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="//gmi:MI_Metadata/gmd:characterSet|//*[@gco:isoType='gmi:MI_Metadata']/gmd:characterSet" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:electronicMailAddress" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template match="gmd:MD_Keywords" mode="snippet-editor">
    ...
    * transformation: current transformation
    ...
    <xsl:variable name="transformation"
                  select="if (count(descendant::gmd:keyword/gmx:Anchor) > 0) then 'to-iso19139.sdn-csr-keyword-with-anchor'
      else if (../@xlink:href) then 'to-iso19139.sdn-csr-keyword-as-xlink'
      else 'to-iso19139.sdn-csr-keyword'"/>
    
    <!-- Define the list of transformation mode available.
    -->
    <xsl:variable name="parentName" select="name(..)"/>
    
    <xsl:variable name="listOfTransformations">'to-iso19139.sdn-csr-keyword', 'to-iso19139.sdn-csr-keyword-with-anchor', 'to-iso19139.sdn-csr-keyword-as-xlink'</xsl:variable>
        ...
        <xsl:apply-templates mode="simpleElement" select=".">
          ...
          <xsl:with-param name="text">
            <xsl:variable name="value">
              ...
              <xsl:if test="$type">
                (<xsl:value-of
                  select="/root/gui/schemas/*[name(.)='iso19139.sdn-csr']/codelists/codelist[@name = 'gmd:MD_KeywordTypeCode']/
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:keyword[following-sibling::gmd:type/gmd:MD_KeywordTypeCode/@codeListValue='place']">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:dateTime|gmd:usageDateTime|gmd:plannedAvailableDateTime" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:date[gco:DateTime|gco:Date]|gmd:editionDate|gmd:dateOfNextUpdate" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gml:*[gml:beginPosition|gml:endPosition]|gml:TimeInstant[gml:timePosition]" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    <xsl:param name="title">
      <xsl:call-template name="getTitle">
  ...
  <xsl:template mode="iso19139.sdn-csr" match="*[geonet:info/isTemplate='s']" priority="3">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="@gco:isoType"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmi:MI_Metadata|*[@gco:isoType='gmi:MI_Metadata']">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$currTab='metadata'">
        <xsl:call-template name="iso19139.sdn-csrMetadata">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="edit"   select="$edit"/>
        </xsl:call-template>
      </xsl:when>
      ...
      <xsl:when test="$currTab='ISOAll'">
        <xsl:call-template name="iso19139.sdn-csrComplete">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="edit"   select="$edit"/>
        </xsl:call-template>
      </xsl:when>
      ...
      <!-- default -->
      <xsl:otherwise>
        <xsl:call-template name="iso19139.sdn-csrSimple">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="edit"   select="$edit"/>
          <xsl:with-param name="flat"   select="/root/gui/config/metadata-tab/*[name(.)=$currTab]/@flat"/>
        </xsl:call-template>
  ...
  <xsl:template name="isotabs">
      ...
      <xsl:call-template name="complexElementGuiWrapper">
        ...
        <xsl:choose>
          <xsl:when test="$dataset=true()">
            <xsl:value-of select="/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmd:MD_DataIdentification']/label"/>
          </xsl:when>
          <xsl:when test="local-name(.)='SV_ServiceIdentification' or contains(@gco:isoType, 'SV_ServiceIdentification')">
            <xsl:value-of select="/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='srv:SV_ServiceIdentification']/label"/>
          </xsl:when>
          <xsl:otherwise>
        ...
        <xsl:if test="$dataset">
          <xsl:for-each select="gmd:extent/gmd:EX_Extent">
            <xsl:call-template name="complexElementGuiWrapper">
              <xsl:with-param name="title" select="/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmd:EX_Extent']/label"/>
              <xsl:with-param name="content">
    ...
    <!-- scope and lineage in their own box -->
      ...
      <xsl:call-template name="complexElementGuiWrapper">
        <xsl:with-param name="title" select="/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmd:LI_Lineage']/label"/>
        <xsl:with-param name="id" select="generate-id(/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmd:LI_Lineage']/label)"/>
        <xsl:with-param name="content">
	  ...
	  <!-- identifier and function in their own box -->
    
      <xsl:call-template name="complexElementGuiWrapper">
        <xsl:with-param name="title" select="/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmd:MD_Identifier']/label"/>
        <xsl:with-param name="id" select="generate-id(/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmd:MD_Identifier']/label)"/>
        <xsl:with-param name="content">

          <xsl:for-each select="gmi:acquisitionInformation/gmi:MI_AcquisitionInformation/gmi:objective/sdn:SDN_Objective">
            <xsl:apply-templates mode="elementEP" select="gmi:function|geonet:child[string(@name)='function']
              |gmi:identifier|geonet:child[string(@name)='identifier']">
              <xsl:with-param name="schema" select="$schema"/>
              <xsl:with-param name="edit"   select="$edit"/>
            </xsl:apply-templates>
          </xsl:for-each>
    ...
    <!-- referenceSystemInfo in its own box -->
      ...
      <xsl:call-template name="complexElementGuiWrapper">
        <xsl:with-param name="title" select="/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmd:referenceSystemInfo']/label"/>
        <xsl:with-param name="id" select="generate-id(/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmd:referenceSystemInfo']/label)"/>
        <xsl:with-param name="content">
      ...
      <xsl:call-template name="complexElementGuiWrapper">
        <xsl:with-param name="title" select="/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmd:distributionInfo']/label"/>
        <xsl:with-param name="id" select="generate-id(/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmd:distributionInfo']/label)"/>
        <xsl:with-param name="content">
    ...
    <xsl:call-template name="complexElementGuiWrapper">
      <xsl:with-param name="title" select="/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmi:MI_Metadata']/label"/>
      <xsl:with-param name="id" select="generate-id(/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmi:MI_Metadata']/label)"/>
      <xsl:with-param name="content">
      ...
      <xsl:for-each select="gmd:contact">
        ...
        <xsl:call-template name="complexElementGuiWrapper">
          <xsl:with-param name="title" select="/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmd:contact']/label"/>
          <xsl:with-param name="content">
  ...
  <xsl:template name="iso19139.sdn-csrComplete">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    ...
    <xsl:apply-templates mode="elementEP" select="gmd:identificationInfo|geonet:child[string(@name)='identificationInfo']
	  ...
	  |gmi:acquisitionInformation|geonet:child[string(@name)='acquisitionInformation']
...
<!-- metadata Extension Information - dead last because its boring and
  ...
  <xsl:template name="iso19139.sdn-csrMetadata">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    <xsl:param name="flat"/>
  ...
  <xsl:template name="iso19139.sdn-csrSimple">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    <xsl:param name="flat"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="//gmd:language[gco:CharacterString]" priority="20">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template name="iso19139.sdn-csrGetIsoLanguage" mode="iso19139.sdn-csrGetIsoLanguage" match="*">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    <xsl:param name="value"/>
      <xsl:param name="ref"/>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:if test="../gmd:locale or ../../gmd:locale">
          <xsl:text> (</xsl:text><xsl:value-of select="string(/root/gui/schemas/iso19139.sdn-csr/labels/element[@name='gmd:locale' and not(@context)]/label)"/>
          <xsl:text>:</xsl:text>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:transferOptions">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:contact|gmd:pointOfContact|gmd:citedResponsibleParty">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:CI_OnlineResource" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    ...
    <xsl:choose>
      <xsl:when test="$edit=true()">
        <xsl:apply-templates mode="iso19139.sdn-csrEditOnlineRes" select=".">
          <xsl:with-param name="schema" select="$schema"/>
        </xsl:apply-templates>
      </xsl:when>
  ...
  <xsl:template mode="iso19139.sdn-csrEditOnlineRes" match="*">
    <xsl:param name="schema"/>
    ...
    <xsl:apply-templates mode="complexElement" select=".">
      ...
      <xsl:with-param name="content">
        ...
        <xsl:apply-templates mode="iso19139.sdn-csr" select="gmd:protocol">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="edit"   select="true()"/>
        </xsl:apply-templates>
        ...
        <xsl:choose>
          <xsl:when test="matches(gmd:protocol[1]/gco:CharacterString,'^WWW:DOWNLOAD-.*-http--download.*') 
            ...
            <xsl:apply-templates mode="iso19139.sdn-csrFileRemove" select="gmd:name/gco:CharacterString|gmd:name/gmx:MimeFileType">
          ...
          <xsl:when test="string(gmd:protocol[1]/gco:CharacterString)='DB:POSTGIS' 
            and string(gmd:name/gco:CharacterString|gmd:name/gmx:MimeFileType)!=''">
            <xsl:apply-templates mode="iso19139.sdn-csrGeoPublisher" select="gmd:name/gco:CharacterString">
          ...
          <xsl:otherwise>
            ...
            <xsl:apply-templates mode="iso19139.sdn-csr" select="gmd:name">
              <xsl:with-param name="schema" select="$schema"/>
              <xsl:with-param name="edit"   select="true()"/>
            </xsl:apply-templates>
          </xsl:otherwise>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:CI_OnlineResource[
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:CI_OnlineResource[starts-with(gmd:protocol/gco:CharacterString,'OGC:WMS-') and contains(gmd:protocol/gco:CharacterString,'-get-capabilities') and gmd:name]" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    <xsl:variable name="linkage" select="gmd:linkage/gmd:URL" />
    <xsl:variable name="name" select="normalize-space(gmd:name/gco:CharacterString|gmd:name/gmx:MimeFileType)" />
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:CI_OnlineResource[matches(gmd:protocol/gco:CharacterString,'^WWW:DOWNLOAD-.*-http--download.*') and gmd:name]" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:protocol" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csr" match="gmd:name[name(..)='gmd:CI_OnlineResource']" priority="2">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="iso19139.sdn-csrFileRemove" match="*">
      ...
      <xsl:with-param name="text">
        <table width="100%"><tr>
          ...
          <td align="right">
            ...
            <xsl:if test="$geo">
              <xsl:call-template name="iso19139.sdn-csrGeoPublisherButton">
  ...
  <xsl:template mode="iso19139.sdn-csrGeoPublisher" match="*">
    <xsl:param name="access" select="'public'"/>
    <xsl:param name="id"/>
  ...
  <xsl:template name="iso19139.sdn-csrGeoPublisherButton">
    <xsl:param name="access" select="'public'"/>
    
    <xsl:if test="/root/gui/config/editor-geopublisher">
      <xsl:variable name="bbox">
        <xsl:call-template name="iso19139.sdn-csr-global-bbox"/>
      </xsl:variable>
  ...
  <!-- ===================================================================== -->
  <!-- === iso19139.sdn-csr brief formatting === -->
  <!-- ===================================================================== -->
  ...
  <xsl:template match="iso19139.sdn-csrBrief">
    <metadata>
     <xsl:call-template name="iso19139.sdn-csr-brief"/>
    </metadata>
  </xsl:template>

  <xsl:template name="iso19139.sdn-csr-brief">
      <xsl:variable name="download_check"><xsl:text>&amp;fname=&amp;access</xsl:text></xsl:variable>
  ...
  <!-- In order to add profil specific tabs 
    ...
    <xsl:template name="iso19139.sdn-csr.profileIdCompleteTab">
    <xsl:param name="tabLink"/>
    <xsl:param name="schema"/>
    
    Load iso19139.sdn-csr complete tab if needed
    <xsl:call-template name="iso19139.sdn-csrCompleteTab">
  ...
  <!-- ============================================================================= -->
  <!-- iso19139.sdn-csr complete tab template  -->
  <!-- ============================================================================= -->
  ...
  -->
  <xsl:template mode="iso19139.sdn-csr" match="gmd:country[gco:CharacterString]" priority="1">
    <xsl:param name="schema" />
    <xsl:param name="edit" />
   ...
   <!--
    ...
    -->
    <xsl:template mode="iso19139.sdn-csr" match="gmd:parentIdentifier|gmd:code[name(../..)='gmd:aggregateDataSetIdentifier']"
        priority="2">
        <xsl:param name="schema" />
        <xsl:param name="edit" />
  ...
  <!-- Display extra thumbnails (not managed by GeoNetwork).
    ...
    simple elements.
  -->
  <xsl:template mode="iso19139.sdn-csr" match="gmd:graphicOverview" priority="2">
    <xsl:param name="schema" />
    <xsl:param name="edit" />
  ...
  <!--
    ...
    * ISO 19139 define how to store multilingual content in a metadata
    ...
    <gmd:title xsi:type="gmd:PT_FreeText_PropertyType">
        <gco:CharacterString>Template for Vector data in iso19139.sdn-csr
        (preferred!)</gco:CharacterString>
      <gmd:PT_FreeText>
        <gmd:textGroup>
          <gmd:LocalisedCharacterString locale="#FR">Modèle de saisie pour les données vecteurs en iso19139.sdn-csr</gmd:LocalisedCharacterString>
        </gmd:textGroup>
      </gmd:PT_FreeText>
    </gmd:title>
  ...
  -->  
  <xsl:template mode="iso19139.sdn-csr" match="gmd:locale|geonet:child[string(@name)='locale']" priority="1">
    <xsl:param name="schema" />
    <xsl:param name="edit" />
    <xsl:choose>
      <xsl:when test="$edit = true()">
  ...
  <!-- =====================================================================        
    * Anyway some elements should not be multilingual.
    ...
    !!! WARNING: this is not defined in iso19139.sdn-csr. !!!
