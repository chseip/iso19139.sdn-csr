...
<xsl:stylesheet version="2.0"
  ...
  -->
  <xsl:template mode="mode-iso19139.sdn-csr" match="*[gmd:CI_Date]" priority="2000">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>

    <xsl:apply-templates mode="mode-iso19139.sdn-csr" select="*/gmd:*">
  ...
  <xsl:template mode="mode-iso19139.sdn-csr" match="gmd:dateType" priority="2000"/>
