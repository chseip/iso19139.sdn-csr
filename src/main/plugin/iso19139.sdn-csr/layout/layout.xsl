...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:gn-fn-iso19139.sdn-csr="http://geonetwork-opensource.org/xsl/functions/profiles/iso19139.sdn-csr"
  ...
  <xsl:template mode="mode-iso19139.sdn-csr"
                match="gn:*|@gn:*|@*"
                priority="1000"/>
  ...
  <xsl:template mode="mode-iso19139.sdn-csr" match="gn:child" priority="2000">
  ...
  <xsl:template mode="mode-iso19139.sdn-csr" match="gmd:*|gmx:*|gml:*|srv:*|gts:*|sdn:*">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>

    <xsl:apply-templates mode="mode-iso19139.sdn-csr" select="*|@*">
  ...
  -->
  <xsl:template mode="mode-iso19139.sdn-csr" priority="200"
...
<!--
  ...
  -->
  <xsl:template mode="mode-iso19139.sdn-csr" priority="200" match="*[*/@codeList]">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    ...
    <xsl:call-template name="render-element">
      ...
      <xsl:with-param name="cls" select="local-name()"/>
      <xsl:with-param name="xpath" select="$xpath"/>
      <xsl:with-param name="type" select="gn-fn-iso19139.sdn-csr:getCodeListType(name())"/>
  ...
  -->
  <xsl:template mode="mode-iso19139.sdn-csr" match="*[gn:element/gn:text]">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    <xsl:param name="codelists" select="$iso19139codelists" required="no"/>
  ...
  <xsl:template mode="mode-iso19139.sdn-csr" priority="100" match="gml:*[count(.//gn:element) = 1]">
