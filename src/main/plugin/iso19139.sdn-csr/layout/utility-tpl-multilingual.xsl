...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="get-iso19139.sdn-csr-language">
  ...
  <xsl:template name="get-iso19139.sdn-csr-other-languages-as-json">
    <xsl:variable name="langs">
      ...
      <xsl:otherwise>
        <xsl:variable name="mainLanguage">
          <xsl:call-template name="get-iso19139.sdn-csr-language"/>
        </xsl:variable>
  ...
  <xsl:template name="get-iso19139.sdn-csr-other-languages">
    <xsl:choose>
      <xsl:when test="$metadata/gn:info[position() = last()]/isTemplate = 's'">
