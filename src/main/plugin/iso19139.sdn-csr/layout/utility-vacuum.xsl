...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  * All elements having no child text (ie. normalize-space return '') and
  ...
  -->
  <xsl:function name="gn-fn-iso19139.sdn-csr:vacuum" as="node()">
    <xsl:param name="metadata" as="node()"/>
    <xsl:for-each select="$metadata/*">
      <xsl:apply-templates mode="vacuum-iso19139.sdn-csr"
                           select="."/>
    </xsl:for-each>
  </xsl:function>


  <xsl:function name="gn-fn-iso19139.sdn-csr:isElementOrChildEmpty"
                as="xs:boolean">
  ...
  <xsl:template mode="vacuum-iso19139.sdn-csr"
                match="@*|node()">
    <xsl:variable name="isElementEmpty"
                  select="gn-fn-iso19139.sdn-csr:isElementOrChildEmpty(.)"/>
