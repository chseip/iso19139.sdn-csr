  ...
  <xsl:template match="/">
	...
	<xsl:template match="*" mode="metadata">
      ...
      <xsl:for-each select="srv:coupledResource">
		...
		<xsl:for-each select="gmi:acquisitionInformation/*/gmi:objective//gmd:source[@uuidref]">
			<Field  name="hassource" string="{string(@uuidref)}" store="false" index="true"/>
		</xsl:for-each>
		
		<xsl:for-each select="gmi:acquisitionInformation/*/gmi:objective/*/gmi:identifier">
			<xsl:if test="$inspire='true'">
				<xsl:if test="(
					contains(gmd:DQ_ConformanceResult/gmd:specification/gmd:CI_Citation/gmd:title/gco:CharacterString, '1089/2010') or
					contains(gmd:DQ_ConformanceResult/gmd:specification/gmd:CI_Citation/gmd:title/gco:CharacterString, 'INSPIRE Data Specification') or
		...
		<xsl:for-each select="gmi:acquisitionInformation/*/gmi:objective/*/gmi:function/gco:CharacterString">
			<Field name="function" string="{string(.)}" store="true" index="true"/>
		</xsl:for-each>
